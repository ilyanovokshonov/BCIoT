...
public class Main implements Serializable {
    ...
    public static LinkedHashMap<String, TransactionOutput> UTXOs = new LinkedHashMap<>();
    ...
    public static int walletamountTEST =3;
    public static ArrayList<String> excludednumbers = new ArrayList<>();
    public static Transaction genesisTransaction;

    public static void main(String[] args) throws NotSerializableException, ClassNotFoundException, NoSuchProviderException {
...
//        block1.addTransaction(walletlist.get(0).sendFunds(walletlist.get(1), 35)); //операция выхода через отрицательную транзакцию (то есть запрос средств за вход)
//        addBlock(block1);
        Block block2 = new Block(blockchain.get(blockchain.size() - 1).hash);
        block2.addTransaction(walletlist.get(0).sendFunds(walletlist.get(1), 255)); //операция выхода через отрицательную транзакцию (то есть запрос средств за вход)
//        block2.addTransaction(walletlist.get(0).sendFunds(walletlist.get(2), -13));
        addBlock(block2);
        isChainValid();
...
//        System.out.println(walletlist.get(0).Balance);
        System.out.println(excludednumbers.toString());
        checkcost();
...
//
    ...
    public static void FULL_DESERIALIZE() throws ClassNotFoundException, ClassNotFoundException{
        DeserializeBlock();
        DeserializeWallet();
        DeserializeGenesis();
        DeserializeMap();
    ...
    public static void DeserializeGenesis() throws ClassNotFoundException{
        try
        {
    ...
    public static void DeserializeMap() throws ClassNotFoundException{
        try
        {
            FileInputStream fis = new FileInputStream("Mapdata");
            ObjectInputStream ois = new ObjectInputStream(fis);

            UTXOs = (LinkedHashMap<String, TransactionOutput>) ois.readObject();
...
//            walletlist.get(i).getBalance();
    ...
    public static void SerializeHashMap(LinkedHashMap<String, TransactionOutput> mapname)throws NotSerializableException{
        try {
