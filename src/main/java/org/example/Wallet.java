...
public float getthisBalance() { //перезаписывает балансы у кошельков, к которым применена
    ...
    for (int i=0; i<Main.ids.size();i++){
        boolean result = Main.UTXOs.containsKey(Integer.toString(i));
        if (result ==true){ //попробовать добавить в цикл исключенные числа или другое сравнение
...
//            if (Math.abs(getthisBalance()) < Math.abs(value)) {
        ...
        if (total>value && total-leftover>=0){
